...
class Block:
    def __init__(self, parent, txns, balances, length=1):
        self.id = str(uuid.uuid4())
        self.parent = parent
        self.txns = txns
...
class Simulator:
    def __init__(self, n, z, Ttx, Tks):
        ...
        self.Ttx = Ttx
        self.Tks = Tks
        self.genesis_block = Block(None, {}, {i:0 for i in range(self.n)})
    ...
    def gen_block(self, params, time):
        ...
        new_block = Block(node.longest_chain_block, txns, new_balances, node.longest_chain_block.length+1)
